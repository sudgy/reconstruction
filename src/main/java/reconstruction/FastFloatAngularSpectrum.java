/* This is a minor change to FloatAngularSpectrum included in JDiffraction.
 * The propagation can be optimized by inputting an already FFT'd field, which
 * can already be needed because of filtering.  When filtering, this removes an
 * entire FFT and IFFT calculation.
 *
 * This source file is identical to the FloatAngularSpectrum.java file from
 * jdiffraction v1.2 + commit ec938a4, with the following exceptions:
 *   1. This comment.
 *   2. The package name.
 *   3. The class name.
 *   4. FastFloatAngularSpectrum does not extend FloatPropagator because its
 *      diffract() method's argument is in fourier space, not real space.
 *      This means that the @Override from diffract() has been removed as well.
 *   5. The forward FFT and shift have been removed from diffract().
 */
 ...
 */
public class FastFloatAngularSpectrum {
    ...
    /**
     * Creates a new instance of FastFloatAngularSpectrum. Also performs kernel
     * calculations.
     *
     ...
     */
    public FastFloatAngularSpectrum(int M, int N, float lambda, float z, float dx, float dy) {
        this.M = M;
        this.N = N;
        this.lambda = lambda;
        this.dx = dx;
